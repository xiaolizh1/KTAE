...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1,config=None):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.KTAE:
        token_level_rewards = data.batch['token_level_rewards']
        index = data.non_tensor_batch['uid']
        responses = data.batch['responses']
        response_length = responses.size(-1)
        attention_mask = data.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
        advantages, returns = core_algos.compute_ktae_outcome_advantage_and_keytokens(
            token_level_rewards=token_level_rewards, responses=responses, eos_mask=response_mask, index=index, config=config)
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
...
class RayPPOTrainer(object):
    ...
    def _validate_config(self):
        ...
        if not config.actor_rollout_ref.actor.use_dynamic_bsz:
            ...
            if config.actor_rollout_ref.actor.ppo_micro_batch_size is not None:
                assert (config.actor_rollout_ref.actor.ppo_mini_batch_size %
    ...
    def save_validate(self, inputs, outputs, scores):
        if not os.path.exists(
                f"validate_results/{self.config.trainer.project_name}/{self.config.trainer.experiment_name}"):
            os.makedirs(f"validate_results/{self.config.trainer.project_name}/{self.config.trainer.experiment_name}")
        local_global_step_folder = os.path.join(
            f"validate_results/{self.config.trainer.project_name}/{self.config.trainer.experiment_name}",
            f'global_step_{self.global_steps}_temperature{self.config.actor_rollout_ref.rollout.val_kwargs.temperature}_top_p{self.config.actor_rollout_ref.rollout.val_kwargs.top_p}_top_k{self.config.actor_rollout_ref.rollout.val_kwargs.top_k}.jsonl'
        )
        with open(local_global_step_folder, 'w') as f:
            for input, output, score in zip(inputs, outputs, scores):
                save_data = {'input': input, 'output': output, 'score': score}
    ...
    def _validate(self):
        ...
        self.save_validate(inputs=sample_inputs, outputs=sample_outputs, scores=sample_scores)
    ...
    def _save_checkpoint(self):
        ...
        max_actor_ckpt_to_keep = (self.config.trainer.get("max_actor_ckpt_to_keep", None)
                                  if not remove_previous_ckpt_in_save else 1)
